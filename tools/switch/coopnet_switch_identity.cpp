...
CoopNetSwitchIdentityResult sIdentityResult;

} // namespace

CoopNetSwitchIdentityResult coopnet_switch_hash_file(const char* path) {
    CoopNetSwitchIdentityResult result;
    if (path == nullptr || path[0] == '\0') {
        result.error = COOPNET_SWITCH_IDENTITY_OPEN_FAILED;
        return result;
    }
    ...
    if (end < 0) {
        result.error = COOPNET_SWITCH_IDENTITY_SIZE_FAILED;
        return result;
    }

    const auto unsignedEnd = static_cast<std::uint64_t>(end);
    if (unsignedEnd == 0) {
        result.error = COOPNET_SWITCH_IDENTITY_EMPTY;
        return result;
    }
    ...
    while (result.bytes < unsignedEnd) {
        ...
        if (received <= 0) {
            result.error = COOPNET_SWITCH_IDENTITY_READ_FAILED;
            result.hash = 0;
            return result;
        }
...
CoopNetSwitchIdentityResult coopnet_switch_identity(const char* path) {
    ...
    if (sIdentityCached && sIdentityPath == requestedPath) {
        CoopNetSwitchIdentityResult cached = sIdentityResult;
        cached.cached = true;
        return cached;
    }

    CoopNetSwitchIdentityResult result = coopnet_switch_hash_file(path);
    if (result.error == COOPNET_SWITCH_IDENTITY_OK && result.hash != 0) {
