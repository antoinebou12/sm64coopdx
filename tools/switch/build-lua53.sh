...
if [[ ! -f "${TARBALL}" ]]; then
    if command -v curl >/dev/null 2>&1; then
        ...
        wget --tries=3 --output-document="${TARBALL}" "${URL}"
    else
        echo "curl or wget is required to download Lua ${LUA_VERSION}" >&2
        exit 1
    fi
fi
...
make -C "${SOURCE_DIR}/src" \
    ...
    AR="${AR} rcu" \
    RANLIB="${RANLIB}" \
    MYCFLAGS="-O2 -g -march=armv8-a+crc+crypto -mtune=cortex-a57 -mtp=soft -fPIC -ffunction-sections -fdata-sections -DLUA_USE_POSIX" \
    MYLDFLAGS="" \
    liblua.a
...
echo "Built ${OUTPUT_DIR}/liblua53.a"
